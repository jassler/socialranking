...
You can also install the development version of socialranking from [GitHub](https://github.com/) with:
...
`newPowerRelation()` and `newPowerRelationFromString()` creates a `PowerRelation` object. `createPowerset()`
...
`PowerRelation` objects can be turned into `relations` objects from the [relations](https://cran.r-project.org/web/packages/relations/index.html) package using `powerRelationMatrix()` or `as.relation()`.
