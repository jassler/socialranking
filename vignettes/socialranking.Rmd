...
The remaining social ranking solutions make the lexcel a little more granular by also considering coalition sizes.
More often than not, we may want to attribute a higher value to an element that does better in smaller coalitions than others.
...
For an element $i \in N$, its _appearance matrix_ is the matrix $M^\succsim_i$ with $m$ columns and $|N|$ rows.
Whereas each column $q$ represents an equivalence class, each row $p$ corresponds to the coalition size.

$$(M^\succsim_i)_{p,q} = |\lbrace S \in \Sigma_q: |S| = p\rbrace|$$
:::

Based on the appearance matrix, the remaining lexcel-based ranking solutions can be explored.

::: {.definition data-latex="" text="`r definitionCounter()`" .theorem}
($L^{(1)}$ solution [@2021Lexcel]) The $L^{(1)}$ solution on $\mathcal{T}(\mathcal{P})$ ranks $i$ above $j$, $i R^\succsim_{L^{(1)}}j$, if a $p^0 \in \{1, \dots, n\}$ and $q^0 \in \{1, \dots, m\}$ exists such that the following conditions hold:
...
::: {.definition data-latex="" text="`r definitionCounter()`" .theorem}
($L^{(2)}$ solution [@2021Lexcel]) The $L^{(2)}$ solution on $\mathcal{T}(\mathcal{P})$ ranks $i$ above $j$, $i R^\succsim_{L^{(2)}}j$, if a $p^0 \in \{1, \dots, n\}$ and $q^0 \in \{1, \dots, m\}$ exists such that the following conditions hold:

1. $(M^\succsim_i)_{p,q\hphantom{^0}} = (M^\succsim_j)_{p,q\hphantom{^0}}$ for all $1 \leq p \leq n$ and $1 \leq q < q^0$,
2. Either (2.1) $\sum (M^\succsim_i)_{:,q^0} > \sum (M^\succsim_j)_{:,q^0}$, or (2.2) $(M^\succsim_i)_{p,q^0} = (M^\succsim_j)_{p,q^0}$ for all $1 \leq p < p^0$ and $(M^\succsim_i)_{p^0,q^0} > (M^\succsim_j)_{p^0,q^0}$
:::


::: {.definition data-latex="" text="`r definitionCounter()`" .theorem}
($L^{p}$ solution [@beal2022lexicographic]) For $i, j \in N$, the social ranking solution $L^p$ then ranks $i$ strictly above $i$ if one of the following conditions hold:
...
::: {.definition data-latex="" text="`r definitionCounter()`" .theorem}
($L^{p^*}$ solution [@beal2022lexicographic]) For $i, j \in N$, the social ranking solution $L^{p^*}$ then ranks $i$ strictly above $j$ if one of the following conditions hold:
