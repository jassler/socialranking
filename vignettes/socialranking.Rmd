...
As a slight variation the logical parameter `strictly` calculates $d^*_{ij}(\succeq)$ and $-d^*_{ji}(\succeq)$, the number of coalitions $S \in 2^{N\setminus \{i,j\}}$ where $S\cup\{i\}\succ S\cup\{j\}$.
...
where $\score{KS}(i) = -\max_j d^*_{ji}(\succeq)$ for all $j \in N \setminus \{i\}$.
:::

Recall that $d^*_{ji}(\succeq)$ returns the number of _strict_ relations of $S \cup \{j\} \succ S \cup \{i\}$.
...
There is a small caveat to Definition `r refDef("def:ks")`. For one, it compares the number of strict relation By default this function does not compare $d_{ii}(\succeq)$. In other terms,
