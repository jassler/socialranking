...
It is strongly discouraged to directly manipulate `PowerRelation` objects, since multiple attributes in the object are so tightly coupled, requiring updates in multiple places.
Instead, it is advisable to simply create new `PowerRelation` objects.

For permutating equivalence classes, it is possible to take the equivalence classes in `$eqs` and use a vector of indexes to move them around.
...
# notice that the length of comparators does not need to match length(coalitions)-1
...
The function `doRanking` offers a generic way of creating `SocialRankingSolution` objects.
...
Hence, a `PowerRelation` object is not necessary to create a `SocialRankingSolution` object.
