...
where $\score{KS}(i) = \max_j d_{ji}(\succsim)$ for all $j \in N \setminus \{i\}$.
:::
...
There is a small caveat to Definition `r refDef("def:ks")`. By default this function does not compare $d_{ii}(\succsim)$. In other terms,
the score of every element is the maximum CP-Majority comparison score against all *other* elements.

This is slightly different from the definition found in [@2021Manipulability], where the CP-Majority comparison $d_{ii}(\succsim)$ is also considered. Since by definition  $d_{ii}(\succsim) = 0$, the Kramer-Simpson scores in those cases will never be negative, possibly discarding valuable information.

To still account for the original definition in [@2021Manipulability], the functions `kramerSimpsonScores` and `kramerSimpsonRanking` offer a `compIvsI` parameter that can be set to `TRUE` if one wishes for $d_{ii}(\succsim)$ to be included in the comparisons.
